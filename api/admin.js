...
async function verifyTargetGrowthsDeposit(dep) {
  const transactionId = dep?.provider_reference || dep?.provider_identifier;
  if (!transactionId) return { ok:false, status:"missing_reference" };
  ...
  const providerRef = providerReference(verification, transactionId);
  const payload = {
